...
/* ---------- NEW: Built-in module set (normalized) ---------- */
const BUILTIN_SET = new Set(
  Module.builtinModules
    .map(n => (n.startsWith("node:") ? n.slice(5) : n))
    // কিছু কমন এলিয়াস সেফটির জন্য
    .concat(["fs", "path", "http", "https", "url", "util", "crypto", "os", "stream"])
);

/* ---------- NEW: safer resolver (supports hoisting) ---------- */
function canResolveFromCwd(pkgName) {
  try {
...
module.exports = async function (api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, createLine) {
  ...
  for (const folderModules of folders) {
    const makeColor = folderModules == "cmds" ? createLine("LOAD COMMANDS") : createLine("LOAD COMMANDS EVENT");
      ...
      try {
        ...
        if (allPackage) {
          allPackage = allPackage
            ...
            // ignore absolute/relative/local requires
            .filter(p =>
              p.indexOf("/") !== 0 &&
              p.indexOf("./") !== 0 &&
              p.indexOf("../") !== 0 &&
          ...
          for (let packageName of allPackage) {
            ...
            // ---------- NEW: skip built-in modules ----------
            if (BUILTIN_SET.has(packageName)) {
              // built-in হলে install/track কোনোটাই দরকার নেই
              continue;
            }
            ...
            if (!packageAlready.includes(packageName)) {
              ...
              // use resolver first (more reliable than node_modules/<name>)
              const resolved = canResolveFromCwd(packageName) || existsSync(`${process.cwd()}/node_modules/${packageName}`);
              if (!resolved) {
